...
import { GraphQLScalarType, getDescription, visit, print as print$1, Source, GraphQLSchema, Kind, isSpecifiedScalarType, isIntrospectionType, printType, parse } from 'graphql';
...
function printWithComments(doc) {
    return print(doc);
}
/**
...
function validateInputs(node, existingNode) {
    const printedNode = print$1(node);
    const printedExistingNode = print$1(existingNode);
    ...
    const allNodes = types
        ...
        if (isGraphQLSchema(type)) {
            ...
            const allTypesPrinted = Object.keys(typesMap)
                ...
                if (type.astNode) {
                    return print$1(type.extensionASTNodes ? extendDefinition(type) : type.astNode);
                }
                else {
            ...
            const directivesDeclaration = schema
                .getDirectives()
                .map(directive => (directive.astNode ? print$1(directive.astNode) : null))
                .filter(e => e);
